...
from mouse import ArduinoMouse, GHubMouse
...
class Colorant:
    ...
    def __init__(self, x, y, xfov, yfov, FLICKSPEED, MOVESPEED):
        ...
        self.arduinomouse = GHubMouse()
        self.grabber = Capture(x, y, xfov, yfov)
        self.flickspeed = FLICKSPEED
        self.movespeed = MOVESPEED
        self.switch = int(config['Settings']['SWITCH'], 16)
        self.aim = int(config['Settings']['AM'], 16)
        self.tgr = int(config['Settings']['TR'], 16)
        self.st = int(config['Settings']['ST'], 16)
    ...
    def listen(self):
        while True:
            if win32api.GetAsyncKeyState(self.switch) < 0:
                toggle_window(self)
                time.sleep(0.2)
            if (win32api.GetAsyncKeyState(self.aim) < 0) and self.toggled:
                self.process("move")
            elif (win32api.GetAsyncKeyState(self.tgr) < 0) and self.toggled:
                self.process("click")
            elif (win32api.GetAsyncKeyState(self.st) < 0) and self.toggled:
                self.process("flick")
    ...
    def process(self, action):
        ...
        center = (x + w // 2, y + h // 2)
        y_offset = int(h * float(config['Settings']['HITPOINT']))
